...
if ( isset( $_GET['post'] ) && is_numeric( $_GET['post'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Configuration file for admin context, no data processing.
	...
	$splms_section_post    = get_post( $splms_current_post_id );
	if ( $splms_section_post && SPLMS_POST_TYPES['section'] === $splms_section_post->post_type ) {
		$splms_section_id = $splms_current_post_id;
	}
	...
	$splms_section_post = get_post( $splms_item_id );
	if ( $splms_section_post && SPLMS_POST_TYPES['section'] === $splms_section_post->post_type ) {
		$splms_section_id = $splms_item_id;
	}
} elseif ( isset( $GLOBALS['post'] ) && SPLMS_POST_TYPES['section'] === $GLOBALS['post']->post_type ) {
	// Method 3: Global post context.
	$splms_section_id = $GLOBALS['post']->ID;
}
 ...
 *
 * @param int    $splms_section_id  Section post ID.
 ...
 */
function splms_get_section_field_value( $splms_section_id, $field_id, $group = '', $default_val = '' ) {
	// For new posts or invalid section ID, return default.
	if ( ! $splms_section_id || $splms_section_id <= 0 ) {
		return $default_val;
	}

	// Verify the post exists and is a section.
	$post = get_post( $splms_section_id );
	...
	if ( ! empty( $group ) ) {
		...
		$group_data = get_post_meta( $splms_section_id, $meta_key, true );
		...
		$value    = get_post_meta( $splms_section_id, $meta_key, true );
	...
	array(
		...
		'fields' => array(
			array(
				...
				'default'     => '',
				'value'       => splms_get_section_field_value( $splms_section_id, 'duration', 'section_settings', '' ),
			...
			array(
				...
				'default' => 'beginner',
				'value'   => splms_get_section_field_value( $splms_section_id, 'difficulty_level', 'section_settings', 'beginner' ),
				'column'  => 'half',
				'icon'    => 'star-filled',
				'group'   => 'section_settings',
				'options' => array(
...
return array(
	'sections' => $splms_sections_config,
);
