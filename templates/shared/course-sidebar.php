...
$is_lesson         = ( SPLMS_POST_TYPES['lesson'] === $current_post_type );
$is_quiz           = ( SPLMS_POST_TYPES['quiz'] === $current_post_type );

// Get course information.
$course_id            = null;
$relationships_query  = SkillPulse_LMS_Relationships_Query::get_instance();
$parent_relationships = $relationships_query->get_parents( $current_post_id );
...
if ( $course_id && $is_enrolled ) {
	$curriculum_result = splms_get_course_curriculum( $course_id, $current_user_id, array( 'include_stats' => true ) );
	...
	foreach ( $curriculum_result['sections'] as $section ) {
		...
		foreach ( $section['children'] as $child ) {
			$section_items[] = array(
				...
				'url'       => $child['permalink'],
				'completed' => isset( $child['completed'] ) ? $child['completed'] : false,
	...
	$progress_data = splms_get_course_progress_data( $current_user_id, $course_id );
}
?>
...
<aside class="splms-course-sidebar">
	<div class="splms-course-sidebar__header">
		<h3 class="splms-course-sidebar__title">
			...
			<span><?php echo esc_html( $course_title ? $course_title : __( 'Course Content', 'skillpulse-lms' ) ); ?></span>
		</h3>
		
		<?php
		if ( $is_enrolled && $progress_data['total'] > 0 ) {
			splms_get_template_part(
				...
				array(
					...
					'css_class'        => 'splms-course-sidebar-progress',
				)
			);
		}
		?>
	...
	<?php if ( $is_enrolled && ! empty( $curriculum ) ) { ?>
				...
				<div class="splms-sidebar-section">
					...
					<div class="splms-sidebar-section__items">
						...
						foreach ( $section['items'] as $item ) {
							$is_current   = ( (int) $item['id'] === (int) $current_post_id );
							$item_classes = array( 'splms-sidebar-item' );
							...
							>
								...
								<span class="splms-sidebar-item__content">
									<span class="splms-sidebar-item__title"><?php echo esc_html( $item['title'] ); ?></span>
									<?php if ( isset( $item['duration'] ) && ! empty( $item['duration'] ) ) { ?>
